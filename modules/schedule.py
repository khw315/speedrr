"""Module that manages speed schedules."""
...
class ScheduleModule:
    ...
    def get_reduction_value(self) -> tuple[float, float]:
        ...
        upload_str = '; '.join(
            f'{cfg.start}-{cfg.end}: {format_value(reduction[0])}'
            for cfg, reduction in self.reduction_value_dict.items()
        )
        download_str = '; '.join(
            f'{cfg.start}-{cfg.end}: {format_value(reduction[1])}'
            for cfg, reduction in self.reduction_value_dict.items()
        )

        logger.info("<schedule> Upload reduction values = %s", upload_str)
        logger.info("<schedule> Download reduction values = %s", download_str)
...
class ScheduleThread(threading.Thread):  # pylint: disable=too-many-instance-attributes
        ...
        if isinstance(self._config.upload, str):
            if self._config.upload.lower() == "unlimited":
                ...
                self._upload_reduce_by = (
                    int(self._config.upload[:-1]) / 100 * self._module._config.max_upload  # pylint: disable=protected-access
                )
        else:
        ...
        if isinstance(self._config.download, str):
            if self._config.download.lower() == "unlimited":
                ...
                self._download_reduce_by = (
                    int(self._config.download[:-1]) / 100 * self._module._config.max_download  # pylint: disable=protected-access
                )
        else:
    ...
    def calculate_next_occurrence(self, hour: int, minute: int) -> datetime:
        """Calculate next datetime occurrence for the given hour and minute."""
    ...
    def set_reduction(self):
        ...
        self._module.reduction_value_dict[self._config] = (
            self._upload_reduce_by, self._download_reduce_by
        )
        self._module._update_event.set()  # pylint: disable=protected-access
    ...
    def run(self) -> None:
        while True:
            ...
            next_end_occurrence = self.calculate_next_occurrence(
                self._end_hour, self._end_minute
            )

            logger.debug(
                "<ScheduleThread> Next start occurrence: %s, Next end occurrence: %s",
                next_start_occurrence, next_end_occurrence
            )
            ...
            if next_start_occurrence > next_end_occurrence:
                ...
                logger.debug(
                    "<ScheduleThread> start>end, Sleeping for %s seconds", sleeping_time
                )
                ...
                raise ValueError("Start and end time are the same, this is forbidden.")
